...
if(isset($_POST['action'])){
  ...
  $b = query_str($_POST);
  parse_str($b,$output);
  $action        = lrtrim($output['action']);
  $smtp_username = lrtrim($output['smtp_username']);
  $smtp_password = lrtrim($output['smtp_password']);
  $smtp_server   = lrtrim($output['smtp_server']);
  $smtp_port     = lrtrim($output['smtp_port']);
  $smtp_ssl      = lrtrim($output['smtp_ssl']);
  $xmailer       = lrtrim($output['xmailer']);
  $reconnect     = lrtrim($output['reconnect']);
  $inline        = lrtrim($output['inline']);
  $type          = lrtrim($output['type']);
  $email         = lrtrim($output['mail']);
  $nama          = lrtrim($output['nama']);
  $subject       = lrtrim($output['subject']);
  $pesan         = lrtrim($output['pesan']);
  $emaillist     = strtolower(lrtrim($output['list']));
  $encoding      = lrtrim($output['encode']);
  $file_name     = $_FILES['file']['name'];
  $file_path     = $_FILES['file']['tmp_name'];
  $wait          = lrtrim($output['wait']);

    $pesan   = urlencode($pesan);
    $pesan   = preg_replace("/%5C%22/", "/%22/", $pesan);
...
function change($to,$message_send){
	...
	$tw = strtolower(substr($to, 0, 3));
	$prem = explode('@',$to);
	$emailxx=$tw.'*****@'.$em[1];
	$yusname=ucfirst($prem[0]);
	date_default_timezone_set('GMT');
	$date = date('m/d/Y h:i:s a', time()); 	
	$message_send = str_replace("&domainname&", $frmsite, $message_send);
	$message_send = str_replace("&domain&", $emaildomain, $message_send);
	$message_send = str_replace("&emailxx&", $emailxx, $message_send);
	$message_send = str_replace("&emails&", $url1, $message_send);
	$message_send = str_replace("&email&", $url, $message_send);
	$message_send = str_replace("&date&", $date, $message_send);
	$message_send = str_replace("&name&", $yusname, $message_send);
return $message_send;}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  <meta charset="utf-8">
  <title>Shockx/Mr Spirit Priv8 Mailer New Edited Inbox 2019</title>
        ...
        return jQuery
            ...
            <div class="col-sm-8" style="padding-right: 7.5px !important;">
              ...
              <div style="margin-bottom: 5px" class="input-group">
                    ...
                    <div style="margin-bottom: 5px" class="input-group">
                    <span class="input-group-addon"><i class="glyphicon glyphicon-paperclip"></i><b>Add Number Of Inline-mesage</b></span>
                    <input type="text" name="inline" value="<?php echo $_POST['inline'];?>" style="width: 50px;border-radius: 4px;padding: 3px 6px;">
                  </div>
                   <div style="margin-bottom: 5px" class="input-group">
                    <span class="input-group-addon"><i class="glyphicon glyphicon-paperclip"></i><b>Name of attachment (invoice.doc)</b></span>
                    <input type="text" name="inlineatt" value="<?php echo $_POST['inlineatt'];?>" style="width: 200px;border-radius: 4px;padding: 3px 6px;">
                  </div>
                  
                  
              <div style="margin-bottom: 10px" class="input-group">
                    <span class="input-group-addon"><i class="glyphicon glyphicon-align-justify"></i></span>
                    <textarea class="form-control" rows="10" name="attachdata" placeholder="Attachment Source Code"><?php if(!empty($attachdata)){ echo $attachdata;}?></textarea>
                  </div>
              ...
              <div style="margin-bottom: 10px" class="input-group">
                  ...
                  | Reconnect After                     <font color="white">(emails) 
                  </div>
...
if ($action){
  ...
  if ($_POST['encoding']=='yes') {
    $subject = preg_replace_callback   ('/([^a-z ])/', function($matches) { return sprintf("=%02x",ord(StripSlashes($matches[1])));}, $subject);
        $subject = str_replace(' ', '=20', $subject);
        $subject = "=?utf-8?Q?$subject?=";
        $nama    = preg_replace_callback   ('/([^a-z ])/', function($matches) { return sprintf("=%02x",ord(StripSlashes($matches[1])));}, $nama);
    ...
    $nq=0;
        ...
        for($x=0; $x<$numemails; $x++){
            ...
            if ($to){
				...
				$message_send=change($to,$pesan);
               
                $subject_send = preg_replace("&email&", $todo, $subject);
...
if ($_POST['inline']=="1") {
$mail->AddEmbeddedImage('logo1.png', 'logo1');
}
if ($_POST['inline']=="2") {
$mail->AddEmbeddedImage('logo1.png', 'logo1');
$mail->AddEmbeddedImage('logo2.png', 'logo2');
}
if ($_POST['inline']=="3") {
$mail->AddEmbeddedImage('logo1.png', 'logo1');
$mail->AddEmbeddedImage('logo2.png', 'logo2');
$mail->AddEmbeddedImage('logo3.png', 'logo3');
}
...
if (!empty($_POST['inlineatt'])) {
$my_file = $_POST['inlineatt'];
 //$url=base64_encode($email);
$datas = $_POST['attachdata'];
$data=change($to,$datas);

if(strpos($my_file, 'pdf') !== false){
$datax=makpdf($data,$my_file);
}else{
$datax=$data;	
}
...
fwrite($handle, $datax);
fclose($handle);
$mail->addAttachment("$my_file", "$my_file");			
}
...
<div id="footer">
      <div class="container" align="center">
        <p class="muted credit" style="color: white;"> Priv8 Mailer Inbox 2019 </p>
      </div>
</div>';$mail->SmtpClose();
